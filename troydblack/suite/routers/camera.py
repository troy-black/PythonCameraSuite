...
from troydblack.suite.config import config
...
async def get_stream_video():
    return StreamingResponse(config.camera_driver.stream_image(), media_type='multipart/x-mixed-replace; boundary=frame')
